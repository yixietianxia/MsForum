...
ForumModel.statsFirstPostTime = SELECT MIN(post_time) FROM jforum_posts WHERE post_time is not null
ForumModel.statsFirstRegisteredUserTime = SELECT MIN(user_regdate) FROM jforum_users WHERE user_regdate is not null
